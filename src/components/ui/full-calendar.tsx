...
import DialogContentComponent from './dialog-event'
...
>(({ children, view, ...props }, ref) => { // Added the ref parameter here
  ...
  return (
    <Button
      ...
      size="sm"
      variant="ghost"
      ref={ref} // Pass the ref to the Button component
      {...props}
      onClick={() => {
        setView(view)
        onChangeView?.(view)
              ...
              <Tooltip key={event.id}>
                <TooltipTrigger asChild>
                  <div
                    ...
                    className={cn('relative cursor-pointer p-2 rounded-md text-white', dayEventVariants({ variant: event.color }))}
                    style={{
...
const CalendarDayView = () => {
  ...
  return (
    <div className="flex relative pt-2 overflow-x-hidden overflow-auto h-full">
      <TimeTable />
...
const CalendarWeekView = () => {
  ...
  return (
    <div className="flex flex-col relative overflow-x-hidden overflow-auto h-full">
      ...
      <div className="grid overflow-hidden -mt-px flex-1 auto-rows-fr p-px grid-cols-7 gap-px">
        {monthDates.map((_date) => {
          ...
          return (
            <div
              className={cn(
                'ring-1 p-2 text-sm text-muted-foreground ring-border overflow-x-hidden overflow-auto',
              ...
              {currentEvents.map((event) => {
                return (
                  ...
                  >
                    ...
                    ></div>
                    <span className="flex-1 truncate">{event?.service?.name}</span>
...
const CalendarYearView = () => {
  ...
  return (
    <div className="grid grid-cols-4 gap-10 overflow-x-hidden overflow-auto h-full">
      {months.map((days, i) => (
